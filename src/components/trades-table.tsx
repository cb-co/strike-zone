...
export function TradesTable({ trades, variant, accounts, setups, total, page, perPage, projProfitTotal, netPnlTotal, availableTickers = [] }: Props) {
  ...
  const pageTotalProjProfit = projProfitTotal ?? table.getRowModel().rows.reduce((s, r) => s + (r.original.projectedProfit ?? 0), 0)
  const pageTotalNetPnl = netPnlTotal ?? table.getRowModel().rows.reduce((s, r) => s + (r.original.netPnl ?? 0), 0)

  const activeFilterCount = selectedTickers.length + (dateFrom ? 1 : 0) + (dateTo ? 1 : 0)
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
            ...
            <PopoverTrigger asChild>
              <button className="flex h-7 items-center gap-1.5 rounded-md border border-input bg-input/20 px-2 text-xs/relaxed whitespace-nowrap transition-colors outline-none hover:bg-input/30 focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/30 dark:bg-input/30">
                {selectedTickers.length > 0
                  ...
                  : 'Tickers'}
                <HugeiconsIcon icon={UnfoldMoreIcon} strokeWidth={2} className="size-3.5 text-muted-foreground shrink-0" />
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-52 p-2" align="start">
              <div className="space-y-1 max-h-64 overflow-y-auto">
                {availableTickers.map((ticker) => (
                  <label key={ticker} className="flex items-center gap-2 px-1 py-1 rounded hover:bg-muted cursor-pointer text-sm">
                    <Checkbox
              ...
              {selectedTickers.length > 0 && (
                <Button variant="ghost" className="w-full mt-2 text-xs" onClick={() => navigate({ tickers: '', page: 1 })}>
                  Clear
                </Button>
              )}
            </PopoverContent>
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline">
              Date range
              {(dateFrom || dateTo) && (
                <Badge className="h-4 px-1 text-[10px]">{(dateFrom ? 1 : 0) + (dateTo ? 1 : 0)}</Badge>
              )}
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-64 p-3 space-y-3" align="start">
            ...
            {(dateFrom || dateTo) && (
              <Button variant="ghost" className="w-full text-xs" onClick={() => navigate({ dateFrom: '', dateTo: '', page: 1 })}>
                Clear
              </Button>
            )}
          </PopoverContent>
        ...
        {activeFilterCount > 0 && (
          <Button variant="ghost" className="text-muted-foreground" onClick={() => navigate({ tickers: '', dateFrom: '', dateTo: '', page: 1 })}>
            Clear all
          </Button>
        )}
      </div>
        ...
        <table className="w-full text-sm">
            ...
            {isPending ? (
              ...
              <>
                    ...
                    <td className="px-3 py-2 text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => setEditTrade(row.original)}>Edit</DropdownMenuItem>
                          {variant === 'open' && (
                            <DropdownMenuItem onClick={() => setClosingTrade(row.original)}>Close</DropdownMenuItem>
                          )}
                          {variant === 'open' && row.original.optionType && (
                            <DropdownMenuItem onClick={() => setRollingTrade(row.original)}>Roll</DropdownMenuItem>
                          )}
                          <DropdownMenuSeparator />
                          <DropdownMenuItem variant="destructive" onClick={() => setDeletingTrade(row.original)}>
                            Delete
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                ...
                {table.getRowModel().rows.length === 0 && (
                  <tr>
                    <td colSpan={columns.length + 1} className="px-4 py-8 text-center text-muted-foreground">No trades</td>
                  </tr>
                )}
              </>
            )}
          ...
          {variant === 'open' && total > 0 && (
            <tfoot className="border-t bg-muted/30">
              <tr>
                <td colSpan={7} className="px-3 py-2 text-right text-sm font-medium text-muted-foreground">Total Proj. Profit</td>
                <td className="px-3 py-2 text-right text-sm font-medium">${fmt(pageTotalProjProfit)}</td>
                <td colSpan={4}></td>
              </tr>
            </tfoot>
          )}
          {variant !== 'open' && total > 0 && (
            <tfoot className="border-t bg-muted/30">
              <tr>
                <td colSpan={9} className="px-3 py-2 text-right text-sm font-medium text-muted-foreground">Total Net P&L</td>
                <td className="px-3 py-2 text-right text-sm font-medium">
                  <span className={pageTotalNetPnl >= 0 ? 'text-green-600' : 'text-red-600'}>${fmt(pageTotalNetPnl)}</span>
                </td>
                <td colSpan={5}></td>
              </tr>
            </tfoot>
      ...
      {total > 0 && (
        <div className="flex items-center gap-2">
          ...
          <Select value={String(perPage)} onValueChange={(v) => navigate({ perPage: v, page: 1 })}>
            <SelectTrigger className="w-14"><SelectValue /></SelectTrigger>
            <SelectContent>
              {[10, 20, 50, 100].map((n) => (
          ...
          {totalPages > 1 && (
            <Pagination className="w-auto mx-0 ml-auto justify-end">
              <PaginationContent>
                <PaginationItem>
                  <PaginationPrevious
                    onClick={(e) => { e.preventDefault(); navigate({ page: page - 1 }) }}
                ...
                {pageNumbers(page, totalPages).map((p, i) => (
                  <PaginationItem key={i}>
                    {p === 'ellipsis' ? (
                      ...
                      <PaginationLink
                        onClick={(e) => { e.preventDefault(); navigate({ page: p }) }}
                        isActive={p === page}
